...
const GameControls = memo(function GameControls(props: Props) {
    return (
        <div className="gameControls-container">
            ...
            <button className={`gameControls-button ${props.isButton3Active && "gameControls-button-active"}`} onClick={() => props.button3()}><span>{props.button3Value}{props.button3Value == "All In" ? ` (\$${abbreviateChips(props.playerChips)})`: ""}</span></button>
        </div>
    );
});
